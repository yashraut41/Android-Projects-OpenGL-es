	...
	private void initialize()
	{
		...
		fragmentShaderObject=GLES32.glCreateShader(GLES32.GL_FRAGMENT_SHADER);
		 ...
		 if(iShaderCompileStatus[0]==GLES32.GL_FALSE)
		 {
			...
			//<<<<<<-------------------START VBO Pyramid color ------------------------>>>>>>


			//create vbo buffer for pyramid color
			GLES32.glGenBuffers(1,vboPyramidColor,0 );
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vboPyramidColor[0]);

			//create buffer from our pyramidColor array so we can pass it to glBufferData()
			...
			//unbind vbo buffer of pyramid color
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);

			//<<<<<-----------------------end VBO for pyramid color----------------------->>>> 
			...
			//create vertex array object for CUBE (vao)
			...
			//<<<<<---------START VBO FOR CUBE POSITION ---------------------------->>>>>
			//create buffer for cube position 
			...
			//<<<<<-------------START VBO FOR CUBE COLOR-------------------------->>>>>>>
			GLES32.glGenBuffers(1,vboCubeColor,0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER,vboCubeColor[0]);

			//create buffer from our cubeColor array so we can pass it to glBufferData()
			...
			//now put our array of cube color into these cooked buffer
