...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener,OnDoubleTapListener
{
	...
	private void initialize()
	{
		...
		"uniform int u_lKeyPress ;" +
		 ...
		 if(iShaderCompileStatus[0]==GLES32.GL_FALSE)
		 {
			...
			//<<<<<-------------START VBO FOR CUBE NORMAL-------------------------->>>>>>>
			GLES32.glGenBuffers(1,vboCubeNormal,0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER,vboCubeNormal[0]);

			//create buffer from our cubeNormal array so we can pass it to glBufferData()
			//allocate buffer directly  from native memeory (not VM memory) thats why allocateDirect
	...
	private void display()
	{
		...
		Matrix.translateM(translationaMatrix,0,0.0f,0.0f,-5.0f);
		Matrix.scaleM(scaleMatrix,0,0.75f,0.75f,0.75f);
		Matrix.rotateM(rotationMatrix,0,angleCube,1.0f,1.0f,1.0f);
		
		Matrix.multiplyMM(modelViewMatrix,0,translationaMatrix,0,scaleMatrix,0);
